...
//This is to prevent MBMA auditor's from accessing this page
...
if (!empty($_POST['form_save'])) {
    ...
    $random_invoice_refno = rand(100000, 999999); // Adjust range as needed
    $encounter_id = $encounter; // Ensure this is sanitized and valid
    sqlStatement(
        "UPDATE form_encounter SET invoice_refno = ? WHERE encounter = ?",
        array($random_invoice_refno, $encounter)
    );
    ...
    header("Location: pos_checkout_normal.php");
...
<body>
    ...
    <img src="<?php echo $GLOBALS['webroot']; ?>/sites/default/images/mountain_bend.png" alt="Mountain Bend Medical Associates" style="max-width: 300px; height: auto;">
    <p style="margin: 5px 0; font-family: Arial, sans-serif; font-size: 14px;">
        15166 Rankin Ave, Dunlap, TN 37327, United States <br>
        Phone: +1 423-949-6300
    </p>
</div>
...
function CheckVisible(MakeBlank) { //Displays and hides the check number text box.
    if (document.getElementById('form_method').options[document.getElementById(
            ...
            'form_method').options[document.getElementById('form_method').selectedIndex].value == 'bank_draft'
            || document.getElementById('form_method').options[document.getElementById('form_method').selectedIndex].value == 'credit_card'
        ) {
