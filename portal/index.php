...
$logoSrc = $logoService->getLogo("core/login/primary");
$clientLogo = $logoService->getLogo("core/login/client");
$backgroundImage = $GLOBALS['images_static_relative'] . '/backgrounds/core/login/primary/background.png';
$secondaryImageBackground = $GLOBALS['images_static_relative'] . '/backgrounds/core/login/secondary/background.png';
...
<head>
    ...
    <style>
        ...
        .logoContainer {
            background-image: url(<?php echo ($secondaryImageBackground) ?>);
            background-size: cover;
            background-position: center;
            background-repeat: no-repeat;
            min-height: 200px;
...
<body class="vh-100">
    <div id="container" class="row w-100 h-100">
            ...
            <form class="form pb-5" action="get_patient_info.php" method="POST" onsubmit="return process_new_pass()">
                ...
                <?php if ($_SESSION['pin'] ?? null) { ?>
                    <div class="form-row my-3">
                        ...
                        <div class="col-md">
                            <input class="form-control rounded-lg" name="token_pin" id="token_pin" type="password" autocomplete="none" value="" required pattern=".{6,20}" />
                        </div>
                    </div>
                <?php } ?>
                <div class="form-row my-3">
                    ...
                    <div class="col-md">
                        <input class="form-control rounded-lg" name="pass_new" id="pass_new" type="password" required placeholder="<?php echo xla('Min length is 8 with upper,lowercase,numbers mix'); ?>" pattern="(?=.*\d)(?=.*[a-z])(?=.*[A-Z]).{8,}" />
                    </div>
                </div>
                <div class="form-row my-3">
                    <label class="col-md-2 col-form-label" for="pass_new_confirm"><?php echo xlt('Confirm New Password'); ?></label>
                    <div class="col-md">
                        <input class="form-control rounded-lg" name="pass_new_confirm" id="pass_new_confirm" type="password" required pattern="(?=.*\d)(?=.*[a-z])(?=.*[A-Z]).{8,}" />
                    </div>
                </div>
                <?php if ($GLOBALS['enforce_signin_email']) { ?>
                    <div class="form-row my-3">
                        ...
                        <div class="col-md">
                            <input class="form-control rounded-lg" name="passaddon" id="passaddon" required placeholder="<?php echo xla('Current on record trusted email'); ?>" type="email" autocomplete="none" value="" />
                        </div>
                    </div>
                <?php } ?>
                    ...
                    <fieldset>
                        ...
                        <div class="jumbotron jumbotron-fluid px-5 py-3">
                            <div class="form-row my-3">
                                ...
                                <div class="col-md">
                                    <input type="text" class="form-control rounded-lg" id="fname" required placeholder="<?php echo xla('First Name'); ?>" />
                                </div>
                            </div>
                            <div class="form-row my-3">
                                <label class="col-md-2 col-form-label" for="lname"><?php echo xlt('Last Name') ?></label>
                                <div class="col-md">
                                    <input type="text" class="form-control rounded-lg" id="lname" required placeholder="<?php echo xla('Last Name'); ?>" />
                                </div>
                            </div>
                            <div class="form-row my-3">
                            ...
                            <div class="form-row my-3">
                                ...
                                <div class="col-md">
                                    <input id="emailInput" type="email" class="form-control rounded-lg" required placeholder="<?php echo xla('Current trusted email address on record.'); ?>" maxlength="100" />
                                </div>
                            </div>
                        </div>
        ...
        <?php } else { ?> <!-- Main logon -->
            <div class="bg-white rounded-lg h-100 w-75 mx-auto ">
                <div class="row rounded-lg h-100 shadow-lg">
                    <div class="col-12 col-lg-12 d-flex flex-column logoContainer justify-content-center align-items-center">
                        <img class="img-fluid login-logo" src='<?php echo $clientLogo; ?>'>
                    </div>
                    <div class="col-12 col-lg-12 d-flex flex-column justify-content-center">
                        <div class="container">
                            <div class="row">
                                <div class="col-12">
                                    <div class="d-flex flex-column justify-content-between h-100 p-3 pt-5 px-lg-5 !pb-lg-1 mx-auto" style="max-width: 600px; width:100%;">
                                        <form class="mx-1" action="get_patient_info.php" method="POST" onsubmit="return process()">
                                            ...
                                            <fieldset style="background: transparent !important;">
                                                <legend class="bg-transparent text-center pt-2 py-1">
                                                    <h3><?php echo xlt('Patient Portal Login'); ?></h3>
                                                </legend>
                                                <div class="form-group my-1">
                                                    <label class="col-form-label" for="uname"><?php echo xlt('Username') ?></label>
                                                    <input type="text" class="form-control rounded-lg unames" name="uname" id="uname" value="" autocomplete="none" required />
                                                </div>
                                                <div class="form-group my-1 position-relative">
                                                    <label class="col-form-label" for="pass"><?php echo xlt('Password') ?></label>
                                                    <div class="input-group">
                                                        <input class="form-control rounded-lg" name="pass" id="pass" type="password" required autocomplete="none" />
                                                        <div class="input-group-append">
                                                            <span class="input-group-text bg-white border rounded-lg" onclick="togglePassword()" style="cursor: pointer;">
                                                                <span id="showIcon">
                                                                    <!-- Eye Open Icon (Show Password) -->
                                                                    <svg width="20" height="20" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                                                                        <path d="M12 4.5C6.995 4.5 2.732 7.61 1 12c1.732 4.39 5.995 7.5 11 7.5s9.268-3.11 11-7.5c-1.732-4.39-5.995-7.5-11-7.5zm0 12.5a5 5 0 1 1 0-10 5 5 0 0 1 0 10zm0-8a3 3 0 1 0 .001 6.001A3 3 0 0 0 12 9z" fill="currentColor" />
                                                                    </svg>
                                                                </span>
                                                                <span id="hideIcon" style="display: none;">
                                                                    <!-- Eye Slash Icon (Hide Password) -->
                                                                    <svg width="20" height="20" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                                                                        <path d="M3 3l18 18M10.586 7.414A5.002 5.002 0 0116.585 11M12 4.5c-5.005 0-9.268 3.11-11 7.5a11.755 11.755 0 004.623 5.494M21 12c-1.732-4.39-5.995-7.5-11-7.5a10.944 10.944 0 00-2.385.268M8.465 15.535A5 5 0 0111.413 7.59" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" />
                                                                    </svg>
                                                                </span>
                                                            </span>
                                                        </div>
                                                 ...
                                                 <?php if ($GLOBALS['enforce_signin_email']) { ?>
                                                    <div class="form-group my-1">
                                                        ...
                                                        <input class="form-control rounded-lg passaddons" name="passaddon" id="passaddon" type="email" value="" autocomplete="none" />
                                                    </div>
                                                <?php } ?>
                                                <?php if ($GLOBALS['language_menu_login']) { ?>
                                                    <?php if (count($result3) != 1) { ?>
                                                        <div class="form-group my-1 mb-5">
                                                            <label class="col-form-label" for="selLanguage"><?php echo xlt('Language'); ?></label>
                                                            <select class="form-control rounded-lg" id="selLanguage" name="languageChoice">
                                                                <?php
                                                    ...
                                                    <?php }
                                                        ...
                                                        <div class="col justify-content-center px-0">
                                                            ...
                                                            <?php if (!empty($GLOBALS['portal_two_pass_reset']) && !empty($GLOBALS['google_recaptcha_site_key']) && !empty($GLOBALS['google_recaptcha_secret_key']) && isset($_GET['w']) && (isset($_GET['u']) || isset($_GET['p']))) { ?>
                                                                <button class="btn btn-danger ml-2" onclick="location.replace('./index.php?requestNew=1&site=<?php echo attr_url($_SESSION['site_id']); ?>
                                                                 <?php if (!empty($redirectUrl)) {
                                                                        echo "&redirect=" . attr_url($redirectUrl);
                                                                    } ?>')">
                                                                    <?php echo xlt('Reset Credentials'); ?>
                                                                </button>
                                                            <?php } ?>
                                        ...
                                        <div class="my-3 mt-lg-5 d-flex flex-column align-items-center">
                                            <img class="img-fluid mb-2" width="80px" src='<?php echo $logoSrc; ?>'>
                                            <small class="" style="font-size: 12px;">Powered by OpenEMR</small>
                                        </div>
                                    </div>
                                </div>
                            </div>
...
<script>
    ...
    <script>
        document.addEventListener("DOMContentLoaded", function () {
            const unames = document.querySelectorAll(".unames");
            const passaddons = document.querySelectorAll(".passaddons");

            unames.forEach((uname) => {
                uname.addEventListener("input", function () {
                    passaddons.forEach((passaddon) => {
                        passaddon.value = this.value;
                    });
                });
            });
        });
